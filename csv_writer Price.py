...
class CSVWriter(FileWriter):
    """Writer to CSV table. В этой версии извлекает только средний чек (average_check).
    Рекурсивный поиск находит теги 'food_service_avg_price' в любых частях JSON,
    включая `context.stop_factors` и вложенные `attributes`.
    """
    ...
    def _data_mapping(self) -> dict[str, Any]:
        ...
        # Добавляем только средний чек
        data_mapping['average_check'] = 'Средний чек'
    ...
    # ------------------ вспомогательные функции для поиска среднего чека ------------------
    def _extract_price_from_name(self, name: str) -> str | None:
        if not name:
            return None
        name_norm = re.sub(r'[\s\u00A0]+', ' ', str(name)).strip()
        # ищем число (возможно с десятичной частью) и возможную валюту в конце строки
        ...
        # fallback — вернуть нормализованную строку целиком
        return name_norm if name_norm else None

    def _find_avg_price_values(self, obj: Any) -> list[str]:
        """Рекурсивно ищем в объекте все значения, которые помечены тегом food_service_avg_price
           или похожими. Возвращаем список найденных name-строк."""
        found: list[str] = []

        def walk(o: Any):
            if isinstance(o, dict):
                # прямой объект с tag/name
                tag = str(o.get('tag', '')).lower()
                if 'food_service_avg_price' in tag or 'avg_bill' in tag or 'avg_price' in tag:
                    # имя может быть в 'name' или внутри атрибутов
                    n = o.get('name') or o.get('value') or ''
                    if n:
                        found.append(str(n))
                # attributes: список объектов
                if 'attributes' in o and isinstance(o['attributes'], list):
                    for a in o['attributes']:
                        if isinstance(a, dict):
                            atag = str(a.get('tag', '')).lower()
                            if 'food_service_avg_price' in atag or 'avg_bill' in atag or 'avg_price' in atag:
                                found.append(str(a.get('name') or a.get('value') or ''))
                # stop_factors чаще — список словарей
                if 'stop_factors' in o and isinstance(o['stop_factors'], list):
                    for sf in o['stop_factors']:
                        if isinstance(sf, dict):
                            sftag = str(sf.get('tag', '')).lower()
                            if 'food_service_avg_price' in sftag or 'avg_bill' in sftag or 'avg_price' in sftag:
                                found.append(str(sf.get('name') or ''))
                # рекурсивно ходим по всем значениям
                for v in o.values():
                    walk(v)
                ...
                # примитивы игнорируем (значения мы ищем через окружение)
                return
    ...
    def _extract_raw(self, catalog_doc: Any) -> dict[str, Any]:
        ...
        # Базовые поля (имя/описание/адрес/координаты и т.д.) — сохраняем, как раньше
        ...
        # Контакты — минимальная обработка (как было)
            ...
            def append_contact(contact_type: str, priority_fields: list[str],
                ...
                for i, contact in enumerate(contacts, 1):
                    ...
                    if data_name in data:
                        ...
                        if self._options.csv.add_comments and getattr(contact, 'comment', None):
        ...
        # ------------------- Извлечение среднего чека (универсально) -------------------
        avg_values = []

        # 1) Найти по каталогу (Pydantic объект) — если содержит контекст или другие секции
        try:
            avg_values.extend(self._find_avg_price_values(getattr(catalog_item, '__dict__', {})))
        except Exception:
            # не критично — продолжим с сырым item
            pass

        # 2) Поиск по сырому item (объект из JSON)
        avg_values.extend(self._find_avg_price_values(item))

        # Очистить и выбрать первое подходящее
        avg_values = [v for v in avg_values if v is not None and str(v).strip() != '']
        avg_str = None
        if avg_values:
            # берём первый и пытаемся извлечь нормальный формат "число + валюта"
            avg_str = self._extract_price_from_name(avg_values[0])

        if avg_str is not None:
            data['average_check'] = avg_str
            logger.debug('Found average_check: %s (from %s)', avg_str, avg_values[0])

        # Schedule & Rubrics (как в оригинале)
