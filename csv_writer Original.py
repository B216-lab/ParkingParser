...
class CSVWriter(FileWriter):
    ...
    def _extract_raw(self, catalog_doc: Any) -> dict[str, Any]:
        ...
        # Попробуем сначала raw item -> context.stop_factors
        stop_factors = None
        if isinstance(item, dict):
            stop_factors = item.get('context', {}).get('stop_factors') or []
        # если Pydantic-модель содержит context -> попробуем и оттуда
        if (not stop_factors or len(stop_factors) == 0) and getattr(catalog_item, 'context', None):
            try:
                stop_factors = getattr(catalog_item.context, 'stop_factors', []) or []
            except Exception:
                stop_factors = stop_factors or []

        # Обычная структура: список словарей с keys 'name', 'tag'
        if stop_factors:
            for sf in stop_factors:
                # получить name и tag в безопасном виде (dict или object)
                if isinstance(sf, dict):
                    name = sf.get('name') or ''
                    tag = (sf.get('tag') or '').lower()
                else:
                    name = getattr(sf, 'name', '') or ''
                    tag = (getattr(sf, 'tag', '') or '').lower()

                # Ищем по тегу, который обычно присутствует
                if 'food_service_avg_price' in tag or 'avg_bill' in tag or 'avg_price' in tag:
                    # нормализовать пробелы (включая NBSP)
                    name_norm = re.sub(r'[\s\u00A0]+', ' ', str(name)).strip()
                    # извлечь число и возможную валюту
                    # примеры name: "Чек 400 ₽", "Чек 350 ₽", "Avg check 12.50 USD"
                    ...
                    if m:
                        ...
                        if not cur and '₽' in name_norm:
                            cur = '₽'
                        avg_check_value = f'{num} {cur}'.strip()
                    else:
                        # если паттерн не сработал — положим всю нормализованную строку
                        avg_check_value = name_norm
                    break  # взяли первый встреченный

        # Записать в data
        if avg_check_value is not None:
            data['average_check'] = avg_check_value
